...
constexpr std::chrono::milliseconds c_miscTaskIntervalMilliSeconds = std::chrono::milliseconds(1000);
...
bool MqttLib::isValidTopicNameForSubscription(std::string_view topic)
{
...
const char *MqttLib::connackString(int connackCode)
{
	return m_mosquittoLib.connackString(connackCode);
}

const char *MqttLib::errorString(int errorCode)
{
	return m_mosquittoLib.errorString(errorCode);
}

const char *MqttLib::reasonString(int reasonCode)
{
	return m_mosquittoLib.reasonString(reasonCode);
}

MqttClient::MqttClient(std::string_view clientId, bool cleanSession, bool verbose)
	: m_verbose{verbose}
{
	...
	m_eventLoopHook.init(c_miscTaskIntervalMilliSeconds, this);
}
...
int MqttClient::setTls(const File &cafile)
{
	...
	const auto result = m_mosquitto.client()->tlsSet(cafile.path().c_str());
...
int MqttClient::setUsernameAndPassword(const std::string &username, const std::string &password)
{
	...
	const auto result = m_mosquitto.client()->usernamePasswordSet(username.c_str(), password.c_str());
...
int MqttClient::connect(const Url &host, int port, int keepalive)
{
	...
	spdlog::info("MqttClient::connect() - blocking call of mosquittopp::connect() took {} µs", elapsedTimeMs);
...
void MqttClient::EventLoopHook::engage(const int socket)
{
	...
	// TODO -> if parent is passed as reference here, compilation fails - understand why
...
void MqttClient::EventLoopHook::disengage()
{
	...
	miscTaskTimer->timeout.disconnectAll();
...
{
	std::vector<std::string> keys(qosByTopicOfActiveSubscriptions.size());
