...
class IMqttLib
{
  ...
  public:
	...
	virtual bool isValidTopicNameForSubscription(std::string_view topic) = 0;
};
...
class MqttLib : public IMqttLib
{
  ...
  public:
	...
	bool isValidTopicNameForSubscription(std::string_view topic) override;
  ...
  protected:
	...
	const char *connackString(int connackCode);
	const char *errorString(int errorCode);
	const char *reasonString(int reasonCode);
...
// TODO -> fix subscription comboboxes, not sure where they broke, they are okay on current main
...
class MqttClient : public IMqttClient
{
  ...
  public:
	MqttClient(std::string_view clientId, bool cleanSession = true, bool verbose = false);
	~MqttClient() = default;
